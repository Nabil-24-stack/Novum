...
const REPAIR_SYSTEM_PROMPT = `You are a code-level QA reviewer for a React web application running in Sandpack.
The page has an error. You will receive the error text, source code, available project files, and optionally a screenshot showing the error visually.

Analyze ALL inputs — error text, source code, and screenshot (if provided) — then provide a complete fix.
...
- EVERY code block you return MUST be a COMPLETE file replacement. Do NOT return partial files, sections, diffs, or snippets.
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.warn("[repair-screenshot] Could not parse AI response as JSON:", text);
    ...
    console.error("[repair-screenshot] Error:", err);
    return Response.json(
